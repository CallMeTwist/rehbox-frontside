      ...
      <div className="hidden lg:flex lg:w-1/2 gradient-hero flex-col justify-between p-12">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[["🏃", "Track sessions"], ["💊", "Follow plans"], ["🏆", "Earn rewards"]].map(([icon, label]) => (
            <div key={label} className="glass rounded-xl p-4 text-center"><div className="text-2xl mb-2">{icon}</div><p className="text-white/80 text-xs">{label}</p></div>
          ))}
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex bg-muted rounded-xl p-1 mb-6">
            {(["client", "physiotherapist"] as const).map((r) => (
              <button key={r} onClick={() => setRole(r)} className={`flex-1 text-sm font-semibold py-2 rounded-lg transition-all ${role === r ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" className="w-full px-4 py-2.5 rounded-xl border border-border bg-card text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">Password</label>
              <div className="relative">
                <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" className="w-full px-4 py-2.5 pr-10 rounded-xl border border-border bg-card text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" />
            ...
            <button type="submit" disabled={loading} className="w-full gradient-primary text-white font-bold py-3 rounded-xl shadow-primary hover:opacity-90 transition-opacity disabled:opacity-50">
              {loading ? "Signing in..." : "Sign in"}
            </button>
          </form>
          <p className="text-center text-sm text-muted-foreground mt-6">
            Don't have an account? <Link to={`/register/${role === "client" ? "client" : "physio"}`} className="text-primary font-semibold hover:underline">Register</Link>
          </p>
