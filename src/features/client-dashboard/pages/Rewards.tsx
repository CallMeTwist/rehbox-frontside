...
const Rewards = () => {
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div><h1 className="font-display font-bold text-2xl">Rewards</h1><p className="text-muted-foreground text-sm mt-1">Earn coins by completing sessions and hitting milestones.</p></div>
          ...
          <div key={reward.id} className={`bg-card rounded-2xl p-5 shadow-card border-2 transition-all ${reward.earned ? "border-success/40" : "border-border"}`}>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1"><p className="font-semibold text-sm">{reward.name}</p>{reward.earned && <span className="badge-approved">Earned</span>}</div>
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
        ...
        <div className="space-y-2 text-sm">
          {[["Complete a session", "50 🪙"], ["7-day streak", "150 🪙"], ["Perfect compliance week", "200 🪙"], ["Refer a friend", "300 🪙"]].map(([action, coins]) => (
            <div key={action} className="flex justify-between items-center p-3 rounded-xl bg-muted/50"><span className="text-muted-foreground">{action}</span><span className="font-bold">{coins}</span></div>
          ))}
        </div>
      </div>
    </div>
